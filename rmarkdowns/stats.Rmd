...
```{r, warning=FALSE, message=FALSE}
goal_3_and_11 <- ggplot(sdr_data, aes(x = goal_3_score, 
                     y = goal_11_score, label = country)) +
  geom_point() +
  geom_smooth() +
  stat_cor() +
  theme_minimal() 
ggplotly(goal_3_and_11)
```
...
- Edit the code chunk above to plot a linear regression of 2 different sdg scores 
  ...
  - Make the plot interactive with ggplotly() so you can hover over the points and see the country, x axis sdg score, and y axis sdg score
    ...
    - **Hint** to be able to tell which point is which country, you will have to add `label = country` in the aes function after you tell ggplot which scores you want on the x and y axis
  ...
  - in 4-6 sentences, come up with a hypotheses for the positive or negative correlation between the 2 goals
      There is a positive correlation between SDG goals 3 and 11 meaning when one increases, the other does as well. Goal 3 is good health and goal 11 is sustainable cities and communities. This trend makes sense. Countries that tend to have the highest scores of both SDGs include Canada, Finland, Spain, etc. I'm assuming that countries with high scores for both SDGs (positive correlation) is explained by the value they place on practicing good health for their people and the places where their people live as well.
      
      - are there any points that don't follow the trend (outliers)
      There are outliers with high goal 3 scores and low goal 11 scores, and vice versa. 
        
        - which country(s) are outliers?
        South Sudan, Central African Republic, Afghanistan, Syrian Arab Republic, etc. 