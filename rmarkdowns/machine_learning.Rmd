...
The functions we are going to use to cluster require that the column being clustered (country), based on the information in all of the other columns (sdg scores) is actually the row names of the dataframe

- So, we simply shift the country column to become the row names of the dataframe with this code
...
- What is the models prediction of maternal mortality scores
...
```{r}
k2 <- kmeans(sdr_data_imputed, centers = 6)
```
...
```{r}
rf_matmort <- randomForest(normalized_score_sdg6_water ~ ., data = sdr_data_imputed, importance = TRUE)
```
...
```{r}
Clean_Water_and_Sanitation <- randomForest(normalized_score_sdg6_water ~ ., data = sdr_data_imputed, importance = TRUE)
```
...
```
```{r}
importance_df <- as.data.frame(Clean_Water_and_Sanitation$importance)
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  ...
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Clean Water and Sanitation",
       subtitle = "Top 10",
       y = "SDG Indicator",
       x = "Feature Importance (% Increase in Mean Squared Error)")
```
...
```{r}
pdp::partial(Clean_Water_and_Sanitation, pred.var = "normalized_score_sdg7_elecac", plot = TRUE)

```

```{r}
pd <- pdp::partial(Clean_Water_and_Sanitation, pred.var = c("normalized_score_sdg7_elecac", "normalized_score_sdg6_water"))
# Default PDP
plotPartial(pd)
```
